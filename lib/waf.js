...
var createIPSet = function(ipSetName, callback) {
  ...
  waf.getChangeToken({}, function (err, response) {
    if (err) {
      ...
      (function createIPS() {
        waf.getChangeTokenStatus(response, function(err, res) {
          if (err) {
            callback(err, null);
          } else {
            if (res.ChangeTokenStatus === 'PROVISIONED') {
              waf.createIPSet({
...
var updateIPSet = function(ipset, updates, callback) {
  ...
  waf.getChangeToken({}, function(err, response) {
    if (err) {
      ...
      (function updateIPS() {
        waf.getChangeTokenStatus(response, function(err, res) {
          console.log(res.ChangeTokenStatus);
          if (res.ChangeTokenStatus === 'PROVISIONED') {
...
var createIPSetArray = function(callback) {
  var params = {Limit: 1};
...
var createOrUpdateRBLs = function(addresses, source, callback) {
  console.log("Creating or updating RBL for " + source + " with " + addresses.length);
  async.waterfall([
    function(cback) {
      createIPSetArray((e, r) => {
        if (e) {
          cback(e, null);
        } else {
          cback(null, r);
    ...
    function (data, cback) {
      ...
      function (err, ips) {
        if (err) {
          ...
          console.log(ips.length + ' IP Sets in total for ' + source);
        }
        cback(err, ips);
      });
    }
  ...
  function(err, results) {
    if (err) {
      ...
      // Look for RBLs that have capacity, and add addresses to them. Create new IPSets for leftovers.
      async.waterfall([
        function(callback) {
          async.map(results, function(rbl, c) {
            rblname = rbl.Name;
            if (rblname.indexOf(source) != -1) {
              console.log("Found IPSet for " + source + " - " + rblname);
              var ipSetDescriptors = rbl.IPSetDescriptors.map(function(ipsetdesc) { return ipsetdesc.Value});
              ...
              if (leftoverCount > 0) {
                ...
                console.log("No room to add IP descriptors in " + rblname);
              }
            }
          }, function(e, r) {
            callback(e, r);
      ...
      function(e, d) {
        console.log("Update completed for - " + source);
        callback(e, d);
      });
    }
    callback(null, null);
